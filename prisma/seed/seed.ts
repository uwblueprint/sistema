...
import { faker } from '@faker-js/faker';
 ...
 const main = async () => {
    ...
    await seed.user((createMany) => createMany(5, () => ({
    ...
    await seed.location((createMany) => createMany(5, () => {
      const school = faker.helpers.arrayElement(["Lambton Park Community School", 
     ...
     await seed.absence((createMany) => createMany(10, () => ({
      lessonDate: faker.date.future(),
      subject: faker.helpers.arrayElement(["M_AND_M", "STRINGS", "CHOIR", "PERCUSSION"]),
      lessonPlan: faker.internet.url(),
      reasonOfAbsence: faker.lorem.sentence(),
    })));


    await seed.mailingList((createMany) => createMany(10, () => ({
      name: faker.helpers.arrayElement(["M_AND_M", "STRINGS", "CHOIR", "PERCUSSION"]),
      emails: [
